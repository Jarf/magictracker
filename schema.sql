...
CREATE TABLE `magic`.`game` ( `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT , `seasonId` BIGINT UNSIGNED NOT NULL , `winner` BIGINT UNSIGNED NULL DEFAULT NULL , `date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , PRIMARY KEY (`id`)) ENGINE = InnoDB;
...
ALTER TABLE `game` ADD CONSTRAINT `game_to_player` FOREIGN KEY (`winner`) REFERENCES `player`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
